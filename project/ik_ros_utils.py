...
def get_modified_urdf(base_rot_limit=np.pi, base_trans_limit=1.0,
                       save_path="/tmp/iktutorial/stretch.urdf"):
    ...
    joint_base_rotation = urdfpy.Joint(name='joint_base_rotation',
                                        ...
                                        limit=urdfpy.JointLimit(effort=100.0, velocity=1.0,
                                                                lower=-base_rot_limit,
                                                                upper=base_rot_limit))
    modified_urdf._joints.append(joint_base_rotation)
    link_base_rotation = urdfpy.Link(name='link_base_rotation', inertial=None, visuals=None, collisions=None)
    ...
    joint_base_translation = urdfpy.Joint(name='joint_base_translation',
                                        ...
                                        origin=np.eye(4, dtype=np.float64),
                                        limit=urdfpy.JointLimit(effort=100.0, velocity=1.0,
                                                                lower=-base_trans_limit,
                                                                upper=base_trans_limit))
...
# Full-range chain for approach servoing
...
grasp_chain = ikpy.chain.Chain.from_urdf_file(
    get_modified_urdf(base_rot_limit=0.15, base_trans_limit=0.15,
                      save_path="/tmp/iktutorial/stretch_grasp.urdf"))

# Full 3x3 rotation matrix for gripper pointing straight down (wrist_pitch = -π/2).
# Derived from FK so it matches the actual kinematic chain convention.
_q_down = [0.0] * 16
_q_down[4]  = 0.4          # lift — non-zero to avoid degenerate config
_q_down[12] = -np.pi / 2   # wrist_pitch straight down
GRASP_DOWN_ORIENT = grasp_chain.forward_kinematics(_q_down)[:3, :3]
...
def move_to_configuration(node, q):
    ...
    node.move_to_pose(GOAL_POSE, blocking=True)
    # TODO: -------------- end ---------------
